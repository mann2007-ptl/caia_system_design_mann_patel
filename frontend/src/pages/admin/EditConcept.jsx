...
import { getConceptById, updateConcept } from '../../features/admin/adminSlice';
...
const EditConcept = () => {
    ...
    const { currentConcept, loading } = useSelector(state => state.admin);
    ...
    const formik = useFormik({
        ...
        initialValues: {
            prompt: currentConcept?.prompt || '',
            response: currentConcept?.response || '',
            metadata: {
                concept: currentConcept?.metadata?.concept || '',
                category: currentConcept?.metadata?.category || '',
                subcategory: currentConcept?.metadata?.subcategory || '',
                difficulty: currentConcept?.metadata?.difficulty || 'Medium',
                question_type: currentConcept?.metadata?.question_type || 'Concept',
                language: currentConcept?.metadata?.language || 'General'
            }
        },
        validationSchema,
        onSubmit: async (values) => {
            const resultAction = await dispatch(updateConcept({ id, data: values }));
            if (updateConcept.fulfilled.match(resultAction)) {
                toast.success('Concept updated successfully!');
                navigate('/admin/concepts');
            } else {
                toast.error(resultAction.payload || 'Failed to update concept');
            }
        }
    });

    if (loading && !currentConcept) {
        return (
            <div className="dash-page fade-in" style={{ maxWidth: '860px', margin: '0 auto' }}>
                <PageHeader title="Edit Concept" />
                <LoadingSkeleton type="card" />
                <div style={{ marginTop: '1rem' }}><LoadingSkeleton type="card" /></div>
            </div>
        );
    }
    ...
    return (
        <div className="dash-page fade-in" style={{ maxWidth: '860px', margin: '0 auto' }}>
            <Helmet>
                <title>Edit Concept — CAIA Admin</title>
            </Helmet>
                        ...
                        <div style={{
                            ...
                            <div>
                                ...
                                <p style={{ fontSize: '0.85rem', color: 'var(--color-graphite)', marginTop: '0.1rem' }}>
                                    Update the concept metadata and classification details.
                                </p>
                            </div>
                        </div>
                    ...
                    <div style={{ padding: '0 2rem 2rem' }}>
                        ...
                        <div style={{
                            ...
                            <div>
                                ...
                                <p style={{ fontSize: '0.85rem', color: 'var(--color-graphite)', marginTop: '0.1rem' }}>
                                    Edit the prompt and its detailed response. Markdown is supported in the response.
                                </p>
                            </div>
                        </div>
